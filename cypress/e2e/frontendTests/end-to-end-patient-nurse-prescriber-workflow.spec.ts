...
describe("End-to-end workflow tests", () => {
  ...
  it.only("End-to-end workflow for patient registration, nurse consultation, and prescriber medication management with existing recipe", () => {
    ...
    cy.get("@selectedAddress").then((selectedAddress) => {
      ...
      medicalDetailsDropdownSelections.forEach((detail) => {
        onConsultationPage.validateMedicalDetails(detail.label, detail.option);
      });
      ...
      // validate personal details value in the prescriber consultation page.
      personalDetailsAfterEdit.forEach((detail) => {
        onConsultationPage.validatePersonalDetails(detail.label, detail.value);
      });

      //validating screening qna details in prescriber medical details
      cy.get("@screeningQnA").then((questionsData: any) => {
        questionsData.screeningQuestions.forEach((questionsData: any) => {
          cy.log(
            `Validating "${questionsData.answer}" for question: "${questionsData.question}"`
      ...
      //validating medical  qna details in prescriber medical details
      cy.get("@medicalDetailsQnA").then((questionsData: any) => {
        questionsData.medicalQuestions.forEach((questionsData: any) => {
          cy.log(
            `Validating "${questionsData.answer}" for question: "${questionsData.question}"`
      ...
      //validating medical dropdown qna details in prescriber medical details
      medicalDetailsDropdownSelections.forEach((detail) => {
        onConsultationPage.validateMedicalDetails(detail.label, detail.option);
      });

      //validating medical text box details in prescriber medical details
      medicalDetailsTextAreas.forEach((detail) => {
        onConsultationPage.validateMedicalDetails(detail.label, detail.value);
      });

      //validate note details in prescriber medical details
      onConsultationPage.validateNoteDetails(note.title, note.body);
      ...
      onNewRxPage.validatePatientName(
        `${personalDetails.firstName} ${personalDetails.lastName}`
      );
      // onNewRxPage.validateDOB(personalDetails.dateOfBirth); Different date format
      ...
      // Enter quantity
      onNewRxPage.enterQuantity(recipeData.quantity);

      // Enter "4" into "Enter quantity extended i.e 20g"
      onNewRxPage.enterQuantityExtended(recipeData.quantity);
      ...
      onPrescriberDashboardPage.validatePatientIsDisplayed(
        personalDetails.email
      );
    });
  });
